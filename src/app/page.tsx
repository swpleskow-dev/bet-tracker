...
if (!r.ok) {
  ...
  // 1) Batch straight-bets from screenshot
  if (slip?.kind === "batch") {
    ...
    for (const b of batch) {
      ...
      await insert({
        ...
        line: b.bet_type === "moneyline" ? null : (b.line ?? null),
  ...
  if (slip?.bet_type !== "parlay") {
    ...
    if (!game_id) {
      setImportError("Could not match screenshot to a game (teams/date missing or unclear).");
      return;
    }
  ...
  if (legs.length < 2) {
    setImportError("Parsed as parlay but found <2 legs.");
    return;
  }
  ...
  const legsPayload = (legs as any[]).map((l) => {
    ...
    // NOTE: schema doesn't support prop legs; store as text
